...
hikeController.updateOneHike = async (req, res, next) => {
  const { title, difficulty, type, distance, location, date, notes, crowds } =
    req.body;
  const { id } = req.params;
  console.log({ id });

  try {
    const newHike = await Hike.findOneAndUpdate(
      { _id: id },
      { title, difficulty, type, distance, location, date, notes, crowds },
    ).exec();
